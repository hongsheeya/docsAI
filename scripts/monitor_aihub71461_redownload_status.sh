...
INTERVAL="${AIHUB71461_MONITOR_INTERVAL_SEC:-20}"

mkdir -p "$RUN_DIR"

while true; do
  if ! pgrep -f 'redownload_aihub71461_support.sh' >/dev/null 2>&1; then
    exit 0
  fi
...
if match:
    ...
    log_path = Path("/mnt/data/wiz/datasets/logs/aihub_recovery") / f"71461_{filekey}.log"
    log_tail = ""
    phase = "다운로드 중"
    try:
        if log_path.exists():
        ...
        log_tail = ""
    if f"TAR_OK dataset=71461 filekey={filekey}" in log_tail:
        phase = "압축 해제 중"
    if f"DONE_RESUME dataset=71461 filekey={filekey}" in log_tail:
        phase = "파일 완료"
    if f"TAR_INCOMPLETE dataset=71461 filekey={filekey}" in log_tail:
        phase = "이어받기 재시도 중"
    eta_bits = [phase]
    if elapsed_text:
    ...
    data.update({
        ...
        "message": "AI-Hub 71461 자세/행동 지원 데이터 재수신 중입니다.",
...
PY
    "$PY" "$PROJECT/scripts/update_aihub_redownload_status.py" "$STATUS_JSON" "$MAIN_LOG" "71461" "$last_line" || true
  fi
  sleep "$INTERVAL"
done
